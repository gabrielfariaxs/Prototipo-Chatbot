...
import { Monitor, Link2, ChevronDown, Bell, LogOut, ArrowLeft, X } from 'lucide-react'
  ...
  const renderActions = (isCompact = false) => (
    <div className="flex items-center gap-1.5 sm:gap-2 shrink-0 select-none">
      ...
      <div className="shrink-0 flex items-center gap-2 bg-[#fafbfe] border border-[#e6e9f2] px-2 sm:px-2.5 py-1 sm:py-1.5 rounded-[11px] min-w-max select-none">
        ...
        {!isCompact && (
          <div className="hidden xl:flex flex-col shrink-0 text-left">
            <span className="text-xs font-bold text-[#14161f] whitespace-nowrap leading-tight">
              {userName}
            </span>
            <span className="text-[10px] font-semibold text-[#5b6276] whitespace-nowrap leading-tight">
              {userSector}
            </span>
          </div>
        )}
  ...
  return (
    <div className="w-full bg-white border-b border-[#e6e9f2] sticky top-0 z-40 shadow-xs">
      <div className="px-3 sm:px-6 lg:px-8 py-2.5 flex flex-col lg:flex-row lg:items-center justify-between gap-2.5 lg:gap-4">
        ...
        {/* Linha Principal: Menu + Logo + (Ações em telas pequenas) */}
        <div className="flex items-center justify-between w-full lg:w-auto gap-3 shrink-0">
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            ...
            <div className="flex items-center gap-2.5 shrink-0 select-none">
              <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-[11px] bg-[#1f29de] text-white flex items-center justify-center font-bold shadow-xs shrink-0">
          ...
          {/* Em telas menores (< lg), exibe as ações na mesma linha do topo */}
          <div className="flex lg:hidden">
            {renderActions(true)}
          </div>
        </div>

        {/* Abas de navegação (Centro no desktop, linha 2 no mobile/tablet) */}
        <div className="flex items-center overflow-x-auto hide-scrollbar w-full lg:w-auto shrink-0 justify-start lg:justify-center">
          {renderTabs()}
        </div>

        {/* Ações na barra direita (Apenas telas desktop >= lg) */}
